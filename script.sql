...
CREATE TABLE IF NOT EXISTS `Apotek`.`Prescription` (
  ...
  PRIMARY KEY (`idPrescription`, `User_ID`),
...
-- -----------------------------------------------------
-- Table `Apotek`.`User_has_SecKeys`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `Apotek`.`User_has_SecKeys` ;

CREATE TABLE IF NOT EXISTS `Apotek`.`User_has_SecKeys` (
  `User_ID` INT NOT NULL,
  `SecKeys_Key` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`User_ID`, `SecKeys_Key`),
  INDEX `fk_User_has_SecKeys_SecKeys1_idx` (`SecKeys_Key` ASC),
  INDEX `fk_User_has_SecKeys_User1_idx` (`User_ID` ASC),
  CONSTRAINT `fk_User_has_SecKeys_User1`
    FOREIGN KEY (`User_ID`)
    REFERENCES `Apotek`.`User` (`ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_User_has_SecKeys_SecKeys1`
    FOREIGN KEY (`SecKeys_Key`)
    REFERENCES `Apotek`.`SecKeys` (`Key`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
